// Gosterdiyim kimi heart iconlari sehifeden cixdiqdan sonra seclimis olaraq qalmalidir.
// Login olmadiqda wishlist sehifesi acilmamalidir login sehifesine kecid olmalidir.(Toast olaraq) 
// Productlari secdikden sonra derhal logout olduqda productlar secilmis olaraq qalmamalidir.
// Logout olduqda toast olaraq xeberdarliq olmalidir.
let products = [
    {
        "id": 1,
...
function createUserCard() {
    products.forEach((product) => {
        ...
        // let cardDestcription = document.createElement("p");
        // cardDestcription.classList.add("card-description");
...
function toggAddWishlist(productId) {
   if (!currentUser) {
   ...
   if(currentUser.wishlist.some((item) => item.id === productId)){
    let productId = currentUser.wishlist.findIndex(
        (product) => product.id === productId
    );
    currentUser.wishList.splice(productInex, 1);
    user[userIndex] = currentUser;
    localStorage.setItem("users", JSON.stringify(users))
    ...
    heartElment.classList.remove("fa-solid");
    heartElment.classList.add("fa-solid");
